...
def login_mahasiswa():
    ...
    if form.validate_on_submit():
        ...
        if ceknpm and bcrypt.check_password_hash(ceknpm.password, form.password.data):
            ...
            flash('Login Gagal, Periksa NPM dan Password Kembali', 'denger')
...
def edit_mahasiswa():
    ...
    if form.validate_on_submit():
        ...
        form.email.data=current_user.emails
...
def pengaduan():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('Data Berhasil ditambah', 'warning')
...
def update_pengaduan(ed_id):
    form=editpengaduan_F() #registrasi form "editpengaduan_F"
    dt_pengaduan=Tpengaduan.query.get_or_404(ed_id)#cari tahu 404??
    if request.method=="GET":
        ...
        db.session.commit()
        flash('Data Berhasil di edit', 'warning')
        return redirect(url_for('rmahasiswa.pengaduan'))

    return render_template('edit_pengaduan.html', form=form)#form=form(menampilkan edit pengaduan yang berdasarkan form yang telah diregistrasi)
